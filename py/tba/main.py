...
class TBA:
    ...
    def __init__(self, auth_key, auth_id="", auth_secret="", event_key=""):
        ...
        self.session.headers.update(
            {"X-TBA-Auth-Key": auth_key, "X-TBA-Auth-Id": auth_id}
        )
    ...
    def _get(self, url):
        ...
        url = self.READ_URL_PRE + url
        cache_read = read_from_cache(url)
        if cache_read is not None:
            return cache_read

        response = self.session.get(url, headers=extra_headers)
    ...
    def _check_modified(func):
        ...
        def wrapper(
            ...
            **kwargs
            ...
            self._if_modified_since = if_modified_since and datetime.strftime(
    ...
    def teams(self, page=None, year=None, simple=False, keys=False):
        ...
        if page is not None:
            if year:
                if keys:
                    ...
                    return [
                        Team(raw)
                        for raw in self._get(
                            "teams/%s/%s%s" % (year, page, "/simple" if simple else "")
                        )
                    ]
            else:
                if keys:
                    ...
                    return [
                        Team(raw)
                        for raw in self._get(
                            "teams/%s%s" % (page, "/simple" if simple else "")
                        )
                    ]
        # If no page was specified, get all of them and combine.
            ...
            target = 0
            while True:
                page_teams = self.teams(
                    page=target, year=year, simple=simple, keys=keys
                )
                if page_teams:
    ...
    def team(self, team, simple=False):
        ...
        """
        return Team(
            self._get("team/%s%s" % (self.team_key(team), "/simple" if simple else ""))
        )
        ...
        if year:
            if keys:
                ...
                return [
                    ...
                    for raw in self._get(
                        "team/%s/events/%s%s"
                        % (self.team_key(team), year, "/simple" if simple else "")
                    )
                ]
        else:
            if keys:
                ...
                return [
                    ...
                    for raw in self._get(
                        "team/%s/events%s"
                        % (self.team_key(team), "/simple" if simple else "")
                    )
                ]
    ...
    def team_awards(self, team, year=None, event=None):
        ...
        if event:
            return [
                Award(raw)
                for raw in self._get(
                    "team/%s/event/%s/awards" % (self.team_key(team), event)
                )
            ]
        else:
            if year:
                return [
                    Award(raw)
                    for raw in self._get(
                        "team/%s/awards/%s" % (self.team_key(team), year)
                    )
                ]
            else:
                return [
                    Award(raw)
                    for raw in self._get("team/%s/awards" % self.team_key(team))
                ]
    ...
    def team_matches(self, team, event=None, year=None, simple=False, keys=False):
        ...
        if event:
            if keys:
                return self._get(
                    "team/%s/event/%s/matches/keys" % (self.team_key(team), event)
                )
            else:
                return [
                    Match(raw)
                    for raw in self._get(
                        "team/%s/event/%s/matches%s"
                        % (self.team_key(team), event, "/simple" if simple else "")
                    )
                ]
        elif year:
            if keys:
                return self._get(
                    "team/%s/matches/%s/keys" % (self.team_key(team), year)
                )
            else:
                return [
                    Match(raw)
    ...
    def team_districts(self, team):
        ...
        return [
            District(raw)
            for raw in self._get("team/%s/districts" % self.team_key(team))
        ]
    ...
    def team_profiles(self, team):
        ...
        return [
            Profile(raw)
            for raw in self._get("team/%s/social_media" % self.team_key(team))
        ]
    ...
    def team_status(self, team, event):
        ...
        """
        return Status(
            self._get("team/%s/event/%s/status" % (self.team_key(team), event))
        )
    ...
    def events(self, year, simple=False, keys=False):
        ...
        if keys:
            ...
            return [
                Event(raw)
                for raw in self._get(
                    "events/%s%s" % (year, "/simple" if simple else "")
                )
            ]
    ...
    def event_teams(self, event, simple=False, keys=False):
        ...
        if keys:
            ...
            return [
                Team(raw)
                for raw in self._get(
                    "event/%s/teams%s" % (event, "/simple" if simple else "")
                )
            ]
    ...
    def event_matches(self, event, simple=False, keys=False):
        ...
        if keys:
            ...
            return [
                Match(raw)
                for raw in self._get(
                    "event/%s/matches%s" % (event, "/simple" if simple else "")
                )
            ]
    ...
    def match(
        ...
        if key:
            ...
            return Match(
                self._get(
                    "match/{year}{event}_{type}{number}{round}{simple}".format(
                        year=year if not event[0].isdigit() else "",
                        event=event,
                        type=type,
                        number=number,
    ...
    def district_events(self, district, simple=False, keys=False):
        ...
        if keys:
            ...
            return [
                Event(raw)
                for raw in self._get(
                    "district/%s/events%s" % (district, "/simple" if simple else "")
                )
            ]
    ...
    def district_rankings(self, district):
        ...
        """
        return [
            DistrictRanking(raw) for raw in self._get("district/%s/rankings" % district)
        ]
    ...
    def delete_event_matches(self, data=None):
        ...
        return self._post(
            ...
            if data is None
            else "event/%s/matches/delete",
            json.dumps(self.event_key) if data is None else json.dumps(data),
        )
